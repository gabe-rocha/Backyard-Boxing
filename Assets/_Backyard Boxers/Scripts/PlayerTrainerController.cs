...
public class PlayerTrainerController : MonoBehaviour {
    public static PlayerTrainerController Instance { get => instance; set => instance = value; }

    [SerializeField] private Transform bigBagPlayerLookAt, rosterLookAt;
    [SerializeField] private Transform spawnPositionBigBag, spawnPositionForFight;
    ...
    private static PlayerTrainerController instance;
    ...
    private void Start() {
        ...
        model = Instantiate(characterModel, spawnPositionBigBag.position, Quaternion.identity, transform);
        ...
        Data.gameState = Data.GameStates.Training;
    }
    private void Update() {
    ...
    private IEnumerator MoveCoRo(bool left) {
        ...
        while (Time.time < startTime + movementSpeed) {
            transform.RotateAround(bigBagPlayerLookAt.position, Vector3.up, movementStep / movementSpeed * Time.deltaTime);
            yield return null;
        }
    }
    ...
    private void FaceBigBag() {
        if(bigBagPlayerLookAt != null)
            model.transform.LookAt(bigBagPlayerLookAt.position);
    }
    ...
    private IEnumerator OnShowResultsCor() {
        yield return new WaitForSeconds(1f);
        model.transform.position = spawnPositionBigBag.position;
        FaceRosterCamera();
